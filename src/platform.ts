...
import { SmartHQPortableAC } from './devices/portableac.js'
...
export class SmartHQPlatform implements DynamicPlatformPlugin {
  ...
  private async createSmartHQPortableAC(userId: any, device: any, details: any, features: any) {
    const uuid = this.api.hap.uuid.generate(device.applianceId)
    ...
    if (existingAccessory) {
      ...
      if (!device.hide_device) {
        ...
        // create the accessory handler for the restored accessory
        // this is imported from `platformAccessory.ts`
        new SmartHQPortableAC(this, existingAccessory, device)
        this.debugLog(`${device.nickname} uuid: ${device.applianceId}`)
      } else {
        this.unregisterPlatformAccessories(existingAccessory)
      }
      ...
      // create the accessory handler for the newly create accessory
      // this is imported from `platformAccessory.ts`
      new SmartHQPortableAC(this, accessory, device)
      this.debugLog(`${device.nickname} uuid: ${device.applianceId}`)
